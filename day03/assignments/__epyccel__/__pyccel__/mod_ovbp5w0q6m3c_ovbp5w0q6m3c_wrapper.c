...
int64_t bind_c_solve_2d_nonlinearconv_pyccel(int64_t n0_u, int64_t n1_u, double *u, int64_t n0_un, int64_t n1_un, double *un, int64_t n0_v, int64_t n1_v, double *v, int64_t n0_vn, int64_t n1_vn, double *vn, int64_t nt, double dt, double dx, double dy, int64_t c, int64_t k);
...
PyObject *solve_2d_nonlinearconv_pyccel_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!O!O!O!OOOOOO", kwlist, &PyArray_Type, &u_tmp, &PyArray_Type, &un_tmp, &PyArray_Type, &v_tmp, &PyArray_Type, &vn_tmp, &nt_tmp, &dt_tmp, &dx_tmp, &dy_tmp, &c_tmp, &k_tmp))
    {
        return NULL;
    }
    ...
    if (PyIs_NativeInt(k_tmp))
    {
        k = PyInt64_to_Int64(k_tmp);
    }
    else
    {
        PyErr_SetString(PyExc_TypeError, "\"Argument must be native int\"");
    ...
    Out_0001 = bind_c_solve_2d_nonlinearconv_pyccel(nd_ndim(&u, 0), nd_ndim(&u, 1), nd_data(&u), nd_ndim(&un, 0), nd_ndim(&un, 1), nd_data(&un), nd_ndim(&v, 0), nd_ndim(&v, 1), nd_data(&v), nd_ndim(&vn, 0), nd_ndim(&vn, 1), nd_data(&vn), nt, dt, dx, dy, c, k);
...
static PyMethodDef mod_ovbp5w0q6m3c_ovbp5w0q6m3c_methods[] = {
    {
...
static PyModuleDef_Slot mod_ovbp5w0q6m3c_ovbp5w0q6m3c_slots[] = {
...
static struct PyModuleDef mod_ovbp5w0q6m3c_ovbp5w0q6m3c_module = {
    PyModuleDef_HEAD_INIT,
    /* name of module */
    "mod_ovbp5w0q6m3c_ovbp5w0q6m3c",
    /* module documentation, may be NULL */
    NULL,
    /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    0,
    mod_ovbp5w0q6m3c_ovbp5w0q6m3c_methods,
    mod_ovbp5w0q6m3c_ovbp5w0q6m3c_slots
};

/*........................................*/

PyMODINIT_FUNC PyInit_mod_ovbp5w0q6m3c_ovbp5w0q6m3c(void)
{
    import_array();
    return PyModuleDef_Init(&mod_ovbp5w0q6m3c_ovbp5w0q6m3c_module);
}
