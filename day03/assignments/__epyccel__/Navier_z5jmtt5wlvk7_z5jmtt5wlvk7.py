...
def cavity_flow_numba(nt:'int', u:'float[:,:]', v:'float[:,:]', dt:'float', nx:'int', ny:'int', dx:'float', dy:'float', p:'float[:,:]', rho:'int', nu:'float',nit:'int'):
    un = np.empty_like(u)
    vn = np.empty_like(v)
    b = np.zeros((ny, nx))
    ...
    for n in range(nt):
        ...
        pressure_poisson_numba(p, dx, dy, nit)
